...
export const POST: APIRoute = async ({ request, locals }) => {
  try{
    ...
    if(!supervisor_notes) return json({ ok:false, error:"rejection notes required" }, 400);
    ...
    if((r?.meta?.changes ?? 0) === 0) return json({ ok:false, error:"Project must be SUBMITTED to reject." }, 400);
